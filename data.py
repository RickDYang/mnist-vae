...
def load_data(batch_size: int, ratio: float = 0.8):
    ...
    print((f"train_loader: {len(train_loader.dataset)}, val_loader: {len(val_loader.dataset)}, "
...
def slice_data(data_loader, batch_size: int, ratio: float):
    ...
    dataset, _ = torch.utils.data.random_split(data_loader.dataset, [sample_len, len(data_loader.dataset) - sample_len])
